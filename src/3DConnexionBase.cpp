...
namespace controldev{
  ...
  ConnexionBase::ConnexionBase(){
    ...
    r_scale = 350;
    oldValues.setZero();
    deviceID=CONNEXION_SPACE_PILOT_PRO_DEVICE_ID;
  }
  ...
  double& ConnexionBase::axisScaling(Mapping id){
    return scale[id];
  }
  
  int ConnexionBase::getFileDescriptor() {
    return fd;
  }
  
  bool ConnexionBase::init(){
	...
	if((device_info.vendor == CONNEXION_VENDOR_ID) && 
	  (device_info.product == deviceID )) {
  ...
  void ConnexionBase::getValue(struct AnalogValues &coordinates, struct AnalogValues &rawValues) {
    assert(fd > 0);
    rawValues = oldValues;
    ...
    for(i = 0; i < eventCnt; ++i) {
      if(EV_REL == events[i].type || EV_ABS == events[i].type) {
